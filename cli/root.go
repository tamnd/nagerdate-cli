// Package cli assembles the nagerdate command tree from the nagerdate
...
// NewApp assembles the kit application from the nagerdate domain. The
...
//
// To add a command, declare it in nagerdate/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := nagerdate.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(nagerdate.Domain{}).Register(app)
